...
# CAの証明書用の署名要求作成
openssl req -new -key ca-privatekey.pem -out ca-csr.pem -subj "/C=JP/ST=Tokyo/L=Chuo-ku/O=Qass/OU=Qass/CN=127.0.0.1"
# CAの証明書発行
...
# サーバ証明書用の署名要求発行
openssl req -new -key server-privatekey.pem -out server-csr.pem -subj "/C=JP/ST=Tokyo/L=Chuo-ku/O=Qass/OU=Qass/CN=127.0.0.1"
